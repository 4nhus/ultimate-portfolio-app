...
final class ExtensionTests: BaseTestClass {
    ...
    func testIssueTagsUnwrap() {
        let tag = Tag(context: managedOjectContext)
        let issue = Issue(context: managedOjectContext)
    ...
    func testIssueSortingIsStable() {
        let issue1 = Issue(context: managedOjectContext)
        issue1.title = "B Issue"
        issue1.creationDate = .now

        let issue2 = Issue(context: managedOjectContext)
        issue2.title = "B Issue"
        issue2.creationDate = .now.addingTimeInterval(1)

        let issue3 = Issue(context: managedOjectContext)
    ...
    func testTagSortingIsStable() {
        let tag1 = Tag(context: managedOjectContext)
        tag1.name = "B Tag"
        tag1.id = UUID(uuidString: "63320EE8-B088-4E27-A894-726E5ADDF290")

        let tag2 = Tag(context: managedOjectContext)
        tag2.name = "B Tag"
        tag2.id = UUID(uuidString: "FFFFFFFF-B088-4E27-A894-726E5ADDF290")

        let tag3 = Tag(context: managedOjectContext)
        tag3.name = "A Tag"
        tag3.id = UUID()
